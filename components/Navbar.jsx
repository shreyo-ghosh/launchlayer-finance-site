        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0">
            <Link className="flex items-center space-x-2 group" to="/">
              <img className="h-8 w-auto transition-transform duration-300 group-hover:scale-105" src="/Launch-Layer-logo.jpg" alt="LaunchLayer Finance" />
              ...
              {navigation.map((item) => (
                <Link
                  ...
                  className={`relative px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 group ${
                    isActiveLink(item.href) ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:text-blue-600 hover:bg-gray-50'
                  }`}
                >
                  {item.name}
                  <span
                    className={`absolute left-0 bottom-0 h-0.5 w-full bg-blue-600 origin-left transform transition-transform duration-300 ${
